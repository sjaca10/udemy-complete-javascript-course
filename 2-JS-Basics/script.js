...
of two years an argument, executes the steps 2, 3 and 4 and returns a vector
...
Hint: you can use a loop not only to read from an array, like y[i], but also
to set values in an array, like y [i] = ... You can also use the specific
array methods.
*/
...
function calculateAges(yearsOfBirth) {
    ...
    for (var i = 0; i < yearsOfBirth.length; i++) {
        result[i] = calculateAge(yearsOfBirth[i]);
    }
    return result;
}

function isFullAge(years) {
    var fullAge = 18, fullAges = [];
    for (var i = 0; i < years.length; i++) {
        fullAges[i] = years[i] >= 18 ? true : false;
    }
    return fullAges;
}

function printFullAge(years) {
    var ages = calculateAges(years);
    var fullAges = isFullAge(ages);
